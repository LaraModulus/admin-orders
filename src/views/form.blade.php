                                        ...
                                        <address>
                                            <p>
                                                <input type="text" name="names" id="names" class="form-control"
                                                       value="{{old('names', $item->names)}}" placeholder="Client names"
                                                       title="Name">

                                            </p>
                                            <p>
                                                <input type="text" name="phone" id="phone" class="form-control"
                                                       value="{{old('phone', $item->phone)}}" placeholder="Client phone"
                                                       title="Phone">
                                            </p>
                                            <p>
                                                <input type="text" name="email" id="email" class="form-control"
                                                       value="{{old('email', $item->email)}}" placeholder="Client email"
                                                       title="Email">
                                            </p>
                                    ...
                                    <div class="col-md-4">
                                        <p>
                                            <input type="text" name="city" id="city" class="form-control"
                                                   value="{{old('city', $item->city)}}" placeholder="Delivery city"
                                                   title="Delivery city">
                                        ...
                                        <p>
                                            <input type="text" name="country" id="country" class="form-control"
                                                   ...
                                                   placeholder="Delivery country" title="Delivery country">
                                        </p>
                                        <p>
                                            <textarea class="form-control" name="address" id="address"
                                                      title="Delivery address"
                                                      placeholder="Delivery address">{{old('address', $item->address)}}</textarea>
                                        ...
                                        <p>
                                            <select name="payment_method" id="payment_method" class="form-control"
                                                ...
                                                <option value="on_delivery"
                                                        @if($item->payment_method=='on_delivery') selected @endif>On
                                                    delivery
                                                </option>
                                                <option value="bank" @if($item->payment_method=='bank') selected @endif>
                                                    Bank payment
                                                </option>
                                                <option value="card" @if($item->payment_method=='card') selected @endif>
                                                    Card payment
                                                </option>
                                            </select>
                                        </p>
                                ...
                                <div class="row">
                                    <div class="col-xs-12">
                                        <button type="button" class="btn btn-primary btn-sm"
                                                data-ng-click="openItemModal()">Add item
                                        </button>
                                            ...
                                            <tr data-ng-repeat="item in order.items">
                                                <td>
                                                    <button type="button" class="btn btn-xs btn-danger"
                                                            data-ng-click="deleteItem(item.id)"><i
                                                                class="fa fa-trash"></i></button>
                                                    <button class="btn btn-xs btn-success" type="button"
                                                            data-ng-click="openItemModal(item.id)"><i
                                            ...
                                            <tr data-ng-repeat="item in order.history">
                                                <td>
                                                    <button type="button" class="btn btn-xs btn-success"
                                                            data-ng-click="restoreItem(item.id)"><i
                                ...
                                <div class="row">
                                    <div class="col-xs-12">
                                        <textarea class="form-control" name="admin_note" id="admin_note"
                                                  title="Admin notes"
                                                  placeholder="Admin notes (visible only from admins)">{{old('admin_note',$item->admin_note)}}</textarea>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="panel-body">
                            <div class="form-group">
                                ...
                                <select name="status" id="status" class="form-control">
                                    ...
                                    <option value="in_progress" @if($item->status=='in_progress') selected @endif>In
                                        Progress
                                    </option>
                                    <option value="finished" @if($item->status=='finished') selected @endif>Finished
                                    </option>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <select style="width: 100%" name="product_id" id="product_id"
                                    class="form-control select2" data-ng-model="item.product_id"
                                    data-ng-change="changeItemProduct()" aria-hidden="true" tabindex="-1">

                                <option value="@{{ p.id }}" data-ng-repeat="p in products"
                                        data-ng-selected="p.id == item.product_id">@{{ p.title }}</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="product_name">Product name</label>
                            <input type="text" name="product_name" id="product_name" class="form-control"
                                   data-ng-model="item.product_name" title="">
                        </div>
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="product_price" id="product_price" class="form-control"
                                           data-ng-model="item.price" title="">
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="form-group">
                                    <label for="qty">Quantity</label>
                                    <input type="text" name="qty" id="qty" class="form-control" data-ng-model="item.qty"
                                           title="">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="weight" id="weight" class="form-control"
                                   data-ng-model="item.weight" title="">
                        </div>
                        <div class="form-group">
